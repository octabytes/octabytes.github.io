...
input_folder = "./extractor/databases"  # Input folder containing JSON files
static_image_path = "./static/images/databases/{category_id}"  # Path to save images
...
def scrape_data(external_link, link_last_part, category_id):
    ...
    static_image_path_full = f"./static/images/databases/{category_id}/{link_last_part}"
    screenshots = []
    ...
    for count, img in enumerate(screenshot_divs, 1):
        ...
        ext = img_url.split('.')[-1]
        save_path = os.path.join(static_image_path_full, f"screenshot-{count}.{ext}")
...
def process_json_file(file_path, existing_ids):
    """Process a JSON file and return services."""
    with open(file_path, 'r') as f:
        services = json.load(f)
    return services

# List to hold all unique services
all_services = []

# Read existing services to avoid duplication
existing_ids = set()
...
for filename in os.listdir(input_folder):
    if filename.endswith('.json'):
        ...
        for service in services_from_file:
            ...
            link_last_part = service["link"].split('/')[-1]
            ...
            existing_service = next((s for s in all_services if s["id"] == link_last_part), None)

            if existing_service:
                print(f"Service {link_last_part} already exists, adding category_id {category_id}.")
                existing_service["category"]["ids"].append(category_id)
            else:
                # Create new service entry
                print(f"Processing service: {link_last_part}")
                scraped_data = scrape_data(external_link, link_last_part, category_id)

                if scraped_data:  # Check if scraping was successful
                    service_entry = {
                        "category": {
                            "id": category_id,
                            "name": service["category"]["name"],
                            "ids": [category_id]  # Initialize ids with the current category_id
                        },
                        "id": link_last_part,
...
# Save all services to a single file
with open('./extractor/services/databases.json', 'w') as outfile:
